<?php
   ...
   if (!$_GET[topic_id]) {
      header("Location: topiclist_teacher.php");
      exit;
   }
  ...
  if (mysql_num_rows($verify_topic_res) < 1) {
     ...
     $display_block = "<P><em>You have selected an invalid topic.
      Please <a href=\"topiclist_teacher.php\">try again</a>.</em></p>";
  } else {
      //get the topic title
     ...
     while ($posts_info = mysql_fetch_array($get_posts_res)) {
        ...
        $display_block .= "
         ...
         <td width=65% valign=top>$post_text<br><br>
          <a href=\"replytopost_teacher.php?post_id=$post_id\"><strong>REPLY TO
          POST</strong></a></td>
         </tr>";
     }
